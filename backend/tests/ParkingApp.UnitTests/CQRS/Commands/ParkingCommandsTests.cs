...
public class ParkingCommandsTests
{
    ...
    {
        var handler = new UpdateParkingHandler(_mockUow.Object, _mockUpdateLogger.Object);
    ...
    {
        var handler = new DeleteParkingHandler(_mockUow.Object, _mockDeleteLogger.Object);
        var ownerId = Guid.NewGuid();
        var parking = new ParkingSpace { Id = Guid.NewGuid(), OwnerId = ownerId };
        _mockParkingRepo.Setup(r => r.GetByIdAsync(parking.Id, It.IsAny<CancellationToken>())).ReturnsAsync(parking);
        _mockBookingRepo.Setup(r => r.HasBlockingBookingsForSpaceAsync(parking.Id, It.IsAny<DateTime>(), It.IsAny<CancellationToken>())).ReturnsAsync(true);
    ...
    public async Task DeleteParkingHandler_ShouldSucceed()
    {
        ...
        var parking = new ParkingSpace { Id = Guid.NewGuid(), OwnerId = ownerId };
        _mockParkingRepo.Setup(r => r.GetByIdAsync(parking.Id, It.IsAny<CancellationToken>())).ReturnsAsync(parking);
        _mockBookingRepo.Setup(r => r.HasBlockingBookingsForSpaceAsync(parking.Id, It.IsAny<DateTime>(), It.IsAny<CancellationToken>())).ReturnsAsync(false);
    ...
    {
        var handler = new ToggleActiveParkingHandler(_mockUow.Object, _mockToggleLogger.Object);
        var ownerId = Guid.NewGuid();
