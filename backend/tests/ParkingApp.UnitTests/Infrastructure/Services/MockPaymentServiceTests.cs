...
public class MockPaymentServiceTests
{
    ...
    public async Task ProcessPaymentAsync_ReturnsPaymentResult()
    {
        ...
        var request = new PaymentRequest
        {
            ...
            Amount = 50.0m,
            Currency = "USD",
            PaymentMethod = ParkingApp.Marketplace.Contracts.Enums.PaymentMethod.CreditCard,
        ...
        if (result.Success)
        {
            ...
            result.Status.Should().Be(ParkingApp.Marketplace.Contracts.Enums.PaymentStatus.Completed);
            result.ReceiptUrl.Should().NotBeNullOrEmpty();
        }
        else
        {
            result.Status.Should().Be(ParkingApp.Marketplace.Contracts.Enums.PaymentStatus.Failed);
    ...
    public async Task GetPaymentStatusAsync_ReturnsCompleted()
    {
        ...
        // Assert
        result.Should().Be(ParkingApp.Marketplace.Contracts.Enums.PaymentStatus.Completed);
    }
}
