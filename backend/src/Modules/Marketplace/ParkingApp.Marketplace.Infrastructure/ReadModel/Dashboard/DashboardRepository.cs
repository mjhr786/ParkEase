...
namespace ParkingApp.Marketplace.Infrastructure.ReadModel.Dashboard;
...
internal sealed class DashboardRepository : IDashboardRepository
{
    ...
    public async Task<List<DashboardChartDataDto>> GetChartDataAsync(Guid vendorId, CancellationToken ct = default)
    {
        using var connection = _sqlConnectionFactory.CreateConnection();
        const string sql = """
            SELECT TO_CHAR(date_trunc('day', b."CreatedAt"), 'Dy') AS "Label", SUM("TotalAmount") AS "Earnings", CAST(COUNT(b."Id") AS INTEGER) AS "Bookings"
            FROM "Bookings" b
