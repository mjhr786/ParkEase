...
internal class ParkingAvailabilityMlModelService : IParkingAvailabilityModelService
{
    ...
    {
        // Defense in depth: stay idle unless both forecast feature and ML are enabled.
        var forecastOpts = _forecastOptions.CurrentValue;
        if (!forecastOpts.Enabled || !forecastOpts.EnableMl)
        {
            return null;
        }
        ...
        // Reuse one PredictionEngine per cached model (never create per bucket).
        var prediction = modelArtifact.Engine.Predict(MapInferenceInput(input));
    ...
    private async Task<ModelArtifact?> GetOrTrainModelAsync(int intervalMinutes, CancellationToken cancellationToken)
    {
        ...
        if (_memoryCache.TryGetValue<ModelArtifact>(cacheKey, out var cachedArtifact) && cachedArtifact is not null)
        {
        ...
        var hours = Math.Clamp(_forecastOptions.CurrentValue.ModelCacheHours, 1, 72);
        ...
        {
            if (value is ModelArtifact disposed)
                disposed.Dispose();
        });
    ...
    private async Task<ModelArtifact?> TrainModelAsync(int intervalMinutes, CancellationToken cancellationToken)
    {
        ...
        var engine = MlContext.Model.CreatePredictionEngine<ParkingAvailabilityTrainingRow, ParkingAvailabilityPredictionRow>(model);
        return new ModelArtifact(model, engine, trainingRows.Count, modelConfidenceScore);
    }
    ...
    private sealed class ModelArtifact : IDisposable
    {
        public ModelArtifact(
            ITransformer model,
            PredictionEngine<ParkingAvailabilityTrainingRow, ParkingAvailabilityPredictionRow> engine,
        ...
        public PredictionEngine<ParkingAvailabilityTrainingRow, ParkingAvailabilityPredictionRow> Engine { get; }
        public int TrainingRowCount { get; }
        public double ModelConfidenceScore { get; }
