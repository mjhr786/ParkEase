...
using ParkingAvailabilityModelPredictionDto = ParkingApp.Marketplace.Contracts.DTOs.ParkingAvailabilityModelPredictionDto;
using ParkingAvailabilityModelInputDto = ParkingApp.Marketplace.Contracts.DTOs.ParkingAvailabilityModelInputDto;
...
/// <summary>
/// Hybrid availability forecast: off when <see cref="ForecastOptions.Enabled"/> is false;
/// deterministic bookings + historical rates when enabled;
/// ML.NET refinement only when <see cref="ForecastOptions.EnableMl"/> is also true.
/// </summary>
internal sealed class ParkingAvailabilityPredictionService : IParkingAvailabilityPredictionService
{
    ...
    private readonly IOptionsMonitor<ForecastOptions> _forecastOptions;
    ...
    public ParkingAvailabilityPredictionService(
        ...
        IOptionsMonitor<ForecastOptions> forecastOptions,
    ...
    {
        var opts = _forecastOptions.CurrentValue;
        if (!opts.Enabled)
        {
            return new ApiResponse<ParkingAvailabilityForecastDto>(
                false,
                "Availability forecasting is disabled",
                null);
        }

        // ML only when both master forecast and ML switches are on.
        var mlEnabled = opts.Enabled && opts.EnableMl;
        var normalizedHorizon = NormalizeHorizonHours(horizonHours, opts);
        var normalizedInterval = NormalizeIntervalMinutes(intervalMinutes, opts);
        var cacheKey = CacheKeys.ParkingForecast(parkingSpaceId, normalizedHorizon, normalizedInterval, mlEnabled);
        ...
        var ttl = TimeSpan.FromMinutes(Math.Clamp(opts.SingleCacheMinutes, 1, 60));
        await _cache.SetAsync(cacheKey, forecast, ttl, cancellationToken);
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        if (!opts.Enabled)
        {
            return new ApiResponse<List<ParkingAvailabilityForecastDto>>(
                true,
                "Availability forecasting is disabled",
                new List<ParkingAvailabilityForecastDto>());
        }

        var mlEnabled = opts.Enabled && opts.EnableMl;
        var normalizedHorizon = NormalizeHorizonHours(horizonHours, opts);
        var normalizedInterval = NormalizeIntervalMinutes(intervalMinutes, opts);
        var cacheKey = CacheKeys.OwnerForecast(ownerId, normalizedHorizon, normalizedInterval, mlEnabled);
        ...
        var ttl = TimeSpan.FromMinutes(Math.Clamp(opts.OwnerCacheMinutes, 1, 60));
        await _cache.SetAsync(cacheKey, forecasts, ttl, cancellationToken);
        ...
        CancellationToken cancellationToken)
    {
        ...
        for (var index = 0; index < bucketCount; index++)
        {
            ...
            ParkingAvailabilityModelPredictionDto? modelPrediction = null;
            if (mlEnabled)
            {
                modelPrediction = await _modelService.PredictOccupancyAsync(
    ...
    private static int NormalizeHorizonHours(int horizonHours, ForecastOptions opts)
    {
        var max = Math.Clamp(opts.MaxHorizonHours, 1, 48);
        return Math.Clamp(horizonHours, 1, max);
    }

    private static int NormalizeIntervalMinutes(int intervalMinutes, ForecastOptions opts)
    {
        var snapped = intervalMinutes switch
        {
            <= 15 => 15,
            <= 30 => 30,
            _ => 60
        ...
        var min = Math.Clamp(opts.MinIntervalMinutes, 15, 60);
        if (min >= 60)
            return 60;
        if (min >= 30)
