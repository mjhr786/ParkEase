...
using UsagePolicy = ParkingApp.Marketplace.Domain.ValueObjects.UsagePolicy;
using ParkingApp.Marketplace.Contracts.Enums;
using ParkingApp.Marketplace.Domain.Interfaces;
using Duration = ParkingApp.Marketplace.Domain.ValueObjects.Duration;
using PassType = ParkingApp.Marketplace.Domain.ValueObjects.PassType;
using ParkingApp.Identity.Contracts;

namespace ParkingApp.Marketplace.Application.Commands.ParkingPasses;
...
internal sealed class CreateParkingPassHandler : ICommandHandler<CreateParkingPassCommand, ApiResponse<ParkingPassDto>>
{
    private readonly IMarketplaceUnitOfWork _marketplace;
    private readonly IUserLookup _users;
    private readonly ICacheService _cache;

    public CreateParkingPassHandler(IMarketplaceUnitOfWork marketplace, IUserLookup users, ICacheService cache)
    {
    ...
    public async Task<ApiResponse<ParkingPassDto>> HandleAsync(CreateParkingPassCommand command, CancellationToken cancellationToken = default)
    {
        ...
        var user = await _users.GetByIdAsync(command.RequestedByUserId, cancellationToken);
...
internal sealed class AssignCorporatePassHandler : ICommandHandler<AssignCorporatePassCommand, ApiResponse<CorporatePassAssignmentResultDto>>
{
    private readonly IMarketplaceUnitOfWork _marketplace;
    private readonly IUserLookup _users;
    private readonly ICacheService _cache;

    public AssignCorporatePassHandler(IMarketplaceUnitOfWork marketplace, IUserLookup users, ICacheService cache)
    {
        _marketplace = marketplace;
        _users = users;
        _cache = cache;
    ...
    {
        var administrator = await _users.GetByIdAsync(command.RequestedByUserId, cancellationToken);
        if (administrator == null || !administrator.IsActive || !administrator.IsAdmin)
        {
        ...
        var employees = await _users.GetActiveByIdsAsync(employeeIds, cancellationToken);
        ...
        var employeeLookup = employees.ToDictionary(employee => employee.UserId);
