...
/// </summary>
internal class FirebasePushNotificationService : IPushNotificationService
{
    private readonly IDeviceTokenLookup _deviceTokens;
    private readonly ILogger<FirebasePushNotificationService> _logger;

    public FirebasePushNotificationService(
        IDeviceTokenLookup deviceTokens,
    ...
    public async Task<PushResult> SendToUserAsync(
        ...
        CancellationToken cancellationToken = default)
    {
        var tokens = (await _deviceTokens.GetFcmTokensByUserIdAsync(userId, cancellationToken)).ToList();

        if (tokens.Count == 0)
        {
            _logger.LogInformation("No FCM tokens registered for user {UserId} ΓÇö push skipped", userId);
