...
internal sealed class InviteMemberHandler : ICommandHandler<InviteMemberCommand, ApiResponse<InvitationDto>>
{
    ...
    public async Task<ApiResponse<InvitationDto>> HandleAsync(InviteMemberCommand command, CancellationToken ct = default)
    {
        ...
        try
        {
            ...
            var emailAlreadyBelongsToMember = existingUser != null
                && await _corporate.Companies.IsUserMemberAsync(command.CompanyId, existingUser.UserId, ct);
