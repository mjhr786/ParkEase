...
internal sealed class AddMemberHandler : ICommandHandler<AddMemberCommand, ApiResponse<MembershipDto>>
{
    private readonly ICorporateUnitOfWork _corporate;
    private readonly IUserLookup _users;

    public AddMemberHandler(ICorporateUnitOfWork corporate, IUserLookup users)
    {
    ...
    {
        var targetUser = await _users.GetByEmailAsync(command.Dto.Email, ct);
        if (targetUser == null)
        {
