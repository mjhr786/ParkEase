...
public static class CacheInvalidation
{
    ...
    /// Does <b>not</b> bust global search/map by default ΓÇö listing metadata is stable; AvailableSpots
        ...
        bool includeDiscoveryLists = false)
    {
        ...
        {
            cache.RemoveAsync(CacheKeys.Parking(parkingSpaceId), cancellationToken),
            // Space-level availability forecasts (namespace version bump ΓÇö acceptable vs wrong occupancy)
            cache.RemoveByPatternAsync(CacheKeys.ParkingForecastAll, cancellationToken)
        };
    ...
    /// <summary>
    /// Review create/update/delete/owner-response ΓÇö ratings appear on parking detail and discovery.
    /// </summary>
    ...
    /// <summary>Parking pass create/assign ΓÇö pricing depends on active passes.</summary>
