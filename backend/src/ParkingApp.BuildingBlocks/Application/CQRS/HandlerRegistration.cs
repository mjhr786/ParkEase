...
/// Lives in BuildingBlocks so module Application projects need not reference host Application.
/// </summary>
            ...
            foreach (var serviceType in implementation.GetInterfaces())
            {
                ...
                else if (definition == typeof(IQueryHandler<,>))
                {
                    ...
                    queryCount++;
                         ...
                         || definition.FullName == "ParkingApp.Domain.Events.IDomainEventHandler`1")
                {
                    // Register concrete closed handler interface (BuildingBlocks and/or Domain.Events alias).
                    services.AddScoped(serviceType, implementation);
                    eventCount++;
                }
            }
